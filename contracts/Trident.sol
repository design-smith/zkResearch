...
contract Trident is FlashLoanSimpleReceiverBase {
    ...
    function getBalance(address _tokenAddress) view internal {
        uint256 balance = IERC20(_tokenAddress).balanceOf(address(this));
        console.log("Balance of token at address ", Strings.toHexString(uint160(_tokenAddress), 20), ": ", balance);
    }
    ...
    (address executor, GenericRouter.SwapDescription memory swapDesc, bytes memory executorData) = 
        abi.decode(_data, (address, GenericRouter.SwapDescription, bytes));

    TransferHelper.safeApprove(_tokenPath[0], address(aggregationRouterV6), _amountIn);

 
    swapDesc.srcToken = IERC20(_tokenPath[0]);
    swapDesc.dstToken = IERC20(_tokenPath[1]);
    swapDesc.srcReceiver = payable(executor); // Using decoded executor address
    swapDesc.dstReceiver = payable(address(this));
    swapDesc.amount = _amountIn;
    swapDesc.minReturnAmount = _minAmountOut;
    swapDesc.flags = 0; 

    // low level call
    (bool success, bytes memory returnData) = address(aggregationRouterV6).call(
        _data
    );
    
    // Handle the return based on success
    if (success) {
